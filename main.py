...
def main(args):
    ...
    pooling_params = ['pooling_mode', 'pooling_key_weight', 'pooling_key_only_threshold']
    for param in pooling_params:
        if param in default_config:
            agent_config[param] = default_config[param]
    #add
    ablation_type = args.ablation if hasattr(args, 'ablation') else 'full'
    if 'ablation_presets' in default_config:
        if ablation_type in default_config['ablation_presets']:
            ablation_config = default_config['ablation_presets'][ablation_type]
            # Merge the ablation configuration into the agent_config
            agent_config.update(ablation_config)
            print(f"ablation configuration: {ablation_type}")
        else:
            print(f"error ablation configuration")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--map', type=str, default="Foraging-large_coop-v0", help='environment map name',
    ...
    # Agent settings
    parser.add_argument('--agent', type=str, default="teamcomm", help='algorithm name',
                        choices=['teamcomm',
                                 'teamcomm_random',
                                 'tiecomm',
    ...
    parser.add_argument('--moduarity', type=bool, default=True, help='use moduarity_loss')
    ...
    # 新增：池化相关参数
    parser.add_argument('--pooling_mode', type=str, default='key_only',
                        choices=['original', 'weighted', 'key_only'],
                        help='Team pooling mode for inter-team communication')
    parser.add_argument('--pooling_key_weight', type=float, default=1.05,
                        help='Weight multiplier for key nodes in weighted pooling mode')
    parser.add_argument('--pooling_key_only_threshold', type=float, default=0.7,
                        help='Minimum key node ratio for key-only pooling mode')

    parser.add_argument('--ablation', type=str, default='intra_obs_enhance',
                        choices=['full', 'no_intra_key', 'no_inter_key', 'noweigt',
                                 'use_comm_weight_enhancement', 'use_obs_enhancement',
                                 'onlykeypool', 'no_weight_enhancement',
                                 'no_prev_key_for_teaming',
                                 'key_node_only_pool',
                                 'intra_obs_enhance',
                                 'knocom_teamcomm',
                                 'baseline'],
                        help='Key node ablation type')
