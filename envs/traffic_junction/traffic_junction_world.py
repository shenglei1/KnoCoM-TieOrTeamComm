...
class TrafficJunctionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        assert np.all(action <= self.n_action), "Actions should be in the range [0,naction)."
    ...
    def get_obs(self):
        ...
        for i, p in enumerate(self.car_loc):
            # most recent action
            act = self.car_last_act[i] / (self.n_action - 1)
