...
class TerminateIllegalWrapper(BaseWrapper):
    ...
    def step(self, action, observe=True):
        ...
        assert (
            "legal_moves" in self.infos[current_agent]
        ), "Illegal moves must always be defined to use the TerminateIllegalWrapper"
...
class NanZerosWrapper(BaseWrapper):
    ...
    def __init__(self, env):
        super().__init__(env)
        assert all(
            isinstance(space, Box) for space in self.action_spaces.values()
        ), "should only use NanZerosWrapper for Box spaces. Use NanNoOpWrapper for discrete spaces"
...
class NaNRandomWrapper(BaseWrapper):
    ...
    def __init__(self, env):
        super().__init__(env)
        assert all(
            isinstance(space, Discrete) for space in env.action_spaces.values()
        ), "action space should be discrete for NaNRandomWrapper"
        SEED = 0x33BB9CC9
...
class AssertOutOfBoundsWrapper(BaseWrapper):
    ...
    def __init__(self, env):
        super().__init__(env)
        assert all(
            isinstance(space, Discrete) for space in self.action_spaces.values()
        ), "should only use AssertOutOfBoundsWrapper for Discrete spaces"

    def step(self, action, observe=True):
        assert self.action_spaces[self.agent_selection].contains(
            action
...
class ClipOutOfBoundsWrapper(BaseWrapper):
    ...
    def step(self, action, observe=True):
        ...
        if not space.contains(action):
            ...
            ), "action should have shape {}, has shape {}".format(
                space.shape, action.shape
            )
    ...
    * warn on calling step after environment is done
    ...
    def __getattr__(self, value):
        ...
        if value == "agent_order":
            ...
            raise AttributeError(
                "'{}' object has no attribute '{}'".format(type(self).__name__, value)
            )
